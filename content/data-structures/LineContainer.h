 ...
 * License: CC0
 * Source: folklore
 * Description: Convex hull trick / Li Chao-style line container backed by a multiset, maintaining the upper envelope of inserted lines y = mx + b for maximum queries. add(m, b) inserts a line, removing lines it makes obsolete; eval(x) returns the maximum value at x. Watch for overflow in bad()'s cross products.
 * Time: O(\log N) amortized per add, O(\log N) per eval.
 * Status: untested
 */
#pragma once
typedef ll tc;
