 ...
 * License: CC0
 * Source: folklore
 * Description: Minimum-cost maximum flow via successive shortest augmenting paths, using Johnson potentials (an initial SPFA pass to allow negative edge costs, then Dijkstra on reduced costs). min\_cost\_flow(s, t, flow) returns the flow value achieved and its cost; recover() lists saturated original edges.
 * Time: O(VE + F \cdot E \log V), where F is the number of augmenting iterations
 * Status: untested
 */
#pragma once
// O(nm + f * m log n)
