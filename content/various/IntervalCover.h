 ...
 * inclusive] add `or R.empty()` to the marked loop condition. Returns an empty
...
vec<int> cover(pair<T, T> G, vec<pair<T, T>> I) {
	...
	T cur = G.first;
	int at = 0;
	while (cur < G.second) { // (A) add `or R.empty()` for closed intervals
		pair<T, int> mx = make_pair(cur, -1);
		while (at < SZ(I) and I[S[at]].first <= cur) {
