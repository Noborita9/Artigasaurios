 ...
 * License: CC0
 * Source: folklore
 * Description: Unimodal search. ternary() finds the integer minimum of f on
 * the open interval (l,r) by discarding a third each step; golden() does the
 * same over reals using golden-section search for a fixed iteration count.
 * For a real-valued tolerance EPS use iters = log((r-l)/EPS)/log(1.618).
 * Time: O(\log(r-l)) evaluations of f for ternary; O(iters) for golden.
 * Status: untested
 */
#pragma once
// Minimo de `f` en `(l,r)`.
