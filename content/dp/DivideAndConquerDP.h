 ...
 * License: CC0
 * Source: folklore
 * Description: Divide and conquer DP optimization. Computes dp[k][i] = min over j <= i of dp[k-1][j] + cost(j,i), requiring that the optimal split point opt[k][i] is monotone in i. Define cost(j,i) and call calc(j, 1, n, 1, n, calc) for each layer j (1-indexed); relies on external globals dp, cost, n, k, oo, and macro L.
